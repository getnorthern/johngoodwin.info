...
## Full-agent review sweep (July 2026)

All nine agents were run against the whole site in one pass; security and dependency audits came back clean (0 npm vulnerabilities, no secrets, all `target="_blank"` links have `rel="noopener"`). Applied from the findings: a real twinkle-scheduler bug in `ConstellationBackground.vue` (initial durations were seeded as 0, so every node's *first* scheduled twinkle silently never rendered); dot glow gradients are now built in local space and cached by radius+colour instead of ~234 `createRadialGradient` allocations per frame (pixel-identical - the rgb values were already integer-quantised); hot-loop destructuring/array allocations removed; `loading="lazy"` added to the Personal photo; `--nav-overshoot: 200px` token replaced seven hardcoded `200px` literals in AppNav; `prefers-reduced-motion` rules added for the nav/drawer transitions, testimonial-card hover lift, and contact-button hovers; global `:focus-visible` outline and a skip-to-content link added; the eyebrow-only sections (Philosophy, Personal, Testimonials) now use `<h2 class="eyebrow">` (visually identical - `.eyebrow` pins `line-height`) and Highlights gained a visually-hidden `<h2>`, giving the page a complete heading outline; hero tagline prose moved into `profile.ts` (`getHeroTagline()`) to match how all other body copy is stored; Philosophy's `:deep(.eyebrow)` simplified to a plain scoped selector. Deliberately NOT applied: extracting the constellation rAF logic into a composable (single consumer - speculative), moving nav links/contact copy into data files (coin-flip indirection), self-hosting fonts, tokenising the `640px` breakpoint (CSS can't use `var()` in media conditions - CLAUDE.md wording corrected instead), restructuring testimonial card links, and changing `--color-ink-faint` (4.99:1 - passes AA).
